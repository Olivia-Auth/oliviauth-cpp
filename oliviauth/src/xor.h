/**
 * @file xor.h
 * @brief Compile-time string encryption for OliviaAuth library
 *
 ...
 *
 * Usage: RXor("your string here")
 */

#ifndef OLIVIAUTH_XOR_HPP
#define OLIVIAUTH_XOR_HPP
...
#define OLIVIAUTH_APPLY_ROR(c, shift) (((c) >> (shift)) | ((c) << (8 - (shift))))
#define OLIVIAUTH_APPLY_ROL(c, shift) (((c) << (shift)) | ((c) >> (8 - (shift))))

#define OLIVIAUTH_ENCRYPT_CHAR(b, c, seed) \
    OLIVIAUTH_APPLY_ROR(applyXOR((b), (c), static_cast<unsigned char>(seed)), ROR_VALUE)
...
struct EncryptedStringT {
    ...
    constexpr EncryptedStringT(const CharT(&str)[N]) : encrypted_chars{} {
        for (size_t i = 0; i < N - 1; ++i) {
            encrypted_chars[i] = static_cast<CharT>(
                OLIVIAUTH_ENCRYPT_CHAR(LARGE_XOR_SEED,
    ...
    void decrypt(CharT* buffer) const {
        for (size_t i = 0; i < N - 1; ++i) {
            ...
            decrypted_char = OLIVIAUTH_APPLY_ROL(decrypted_char, ROR_VALUE);
...
#define RXor(str) ([]() -> const auto* { \
    using namespace oliviauth::xor_impl; \
